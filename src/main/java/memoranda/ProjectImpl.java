...
public class ProjectImpl implements IProject {
    ...
    public int getStatus() {
        ...
        // SER 316 Complexity change - Method changed from 6 - 5 complexity
        // by removing check on inPeriod. Checking before and after leaves the
        // only other option of being in the period. - ptraxler
       if (today.before(prStart)) {
            return IProject.SCHEDULED;
        }
        
        if (prEnd != null) {
            if (today.after(prEnd)) {
        ...
        // Changed for complexity -ptraxler
        /*if (prEnd == null) {
            if (today.before(prStart))
